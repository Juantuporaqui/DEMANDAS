...
const riesgoConfig: Record<RiskLevel, { bg: string; border: string; text: string; label: string }> = {
  bajo: {
    ...
    >
        ...
        <div className="flex items-start justify-between gap-3">
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-2 flex-wrap">
              ...
              <span className="text-[10px] text-slate-500">{hecho.fecha}</span>
            </div>

            <h3 className="font-semibold text-white text-sm leading-tight mb-1">{hecho.title}</h3>
            <p className="text-xs text-slate-400 line-clamp-2">{hecho.resumenCorto}</p>
      ...
      {expanded && (
        <div className="px-4 pb-4 pt-0 border-t border-slate-700/50 space-y-4">
          ...
          {hecho.tags.length > 0 && (
            <div>
              <h4 className="text-[10px] uppercase tracking-wider text-slate-500 mb-1 font-semibold">Tags</h4>
